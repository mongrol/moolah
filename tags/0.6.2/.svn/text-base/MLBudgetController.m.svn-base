...
-(void)calcAllocation
{
	...
	//	} else if ([expenseType isEqualToString:@"Once"]){
	//		NSTimeInterval timeTillDate = [expenseDate timeIntervalSinceNow];
//			expensePeriod = timeTillDate/60/60/24;
//			NSLog(@"expensePeriod = %d",expensePeriod);
//			NSDecimalNumber *expenseTarget = [envelope target]; 
//			NSLog(@"%@ wants %d %@ on date %@",[envelope valueForKeyPath:@"account.name"],[expenseTarget intValue], expenseType, [expenseDate description]);
//			
//			//calculate the expense allocation
//			//then get the amount required in the envelope
//			NSDecimalNumber *netTarget = [expenseTarget decimalNumberBySubtracting:[envelope balance]];		NSLog(@"netTarget is %@",netTarget);
//			//how many pays between now and then.
//			NSDecimalNumber *periodModifier = [incomePeriod decimalNumberByDividingBy:expensePeriod withBehavior:expenseAllocationHandler];
//			NSLog(@"periodModifier is %@",periodModifier);
//			NSDecimalNumber *expenseAllocationDecimal = [netTarget decimalNumberByMultiplyingBy:periodModifier withBehavior:expenseAllocationHandler];
//			NSLog(@"expenseAllocation is %@",expenseAllocationDecimal);
//			[envelope setValue:expenseAllocationDecimal forKey:@"allocation"];
		}
...
- (void)tableView:(NSTableView *)tableView willDisplayCell:(id)cell forTableColumn:(NSTableColumn *)tableColumn row:(NSInteger)row
{
	...
	if ([tableView isEqualTo:expenseTable]){
		...
		//change date formatter to suit targetType. Reenable this for calcAlloc refactoring.
//		if ([[tableColumn identifier] isEqualToString:@"date"]){
//			NSManagedObject *targetType = [[[expenseEnvelopeController arrangedObjects] objectAtIndex:row] targetType];
//			//NSLog(@"periodString is %@",[targetType name]);
//			if ([[targetType name] isEqualToString:@"Once"]){
//				[cell setEnabled:YES];
//				NSDateFormatter *dateFormat = [[NSDateFormatter alloc] initWithDateFormat:@"%1d/%1m/%1y" allowNaturalLanguage:NO];
//				[cell setFormatter:dateFormat];
//				[cell setEditable:YES];
//			} else if ([[targetType name] isEqualToString:@"Yearly"]){
//				[cell setEnabled:YES];
//				NSDateFormatter *dateFormat = [[NSDateFormatter alloc] initWithDateFormat:@"%1d %b" allowNaturalLanguage:NO];
//				[cell setFormatter:dateFormat];
//				[cell setEditable:YES];
//			} else {
//			//	NSLog(@"disabling date");
//				NSDateFormatter *dateFormat = [[NSDateFormatter alloc] initWithDateFormat:@"" allowNaturalLanguage:NO];
//				[cell setFormatter:dateFormat];
//				[cell setEnabled:NO];
//			}
//		}
