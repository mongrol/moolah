...
- (void) importFile:(NSString *)aFile{
	...
	NSMutableArray *importFileRows = [importFile componentsSeparatedByString:@"\n"];
	
	//Remove last object as objectEnumerator barfs on EOF
	[importFileRows removeLastObject];
	
	//enumerate the array and extract the data. We tst each row and based on the starting char
	// plonk that data into the correct field in a dictionary. If we hit a ^, its the end of a
	...
	self.importedTransactions = [unsortedTransactions sortedArrayUsingDescriptors:sortDescriptors];
	NSLog(@"Imported transactions count=%d",[self.importedTransactions count]);
